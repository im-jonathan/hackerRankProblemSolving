# The Kingdom of Gridland contains P provinces. Each province is defined as a 2xN grid where each cell in the grid represents a city. Every cell in the grid contains a single lowercase character denoting the first character of the city name corresponding to that cell.
...
# Input Format
# The first line contains a single integer, P, denoting the number of provinces. The 3xP subsequent lines describe each province over the following three lines:
...
def hrange(l: list, i: int, j: int, s: int) -> int:
    return (l[j] + ((s-l[i]) * PMAP[j-i])) % PRIME


def count(s1: str, s2: str, s1rev: str, s2rev: str) -> set:
    ret = set()
    ...
    mid_even_tb = [ord(s1[i//2]) if ((i % 4) in (0, 3))
                   else ord(s2[i//2]) for i in range(n2)]
    mid_odd_tb = [ord(s2[i//2]) if ((i % 4) in (0, 3))
                  else ord(s1[i//2]) for i in range(n2)]
    left_to_top, left_to_bot, right_from_top, right_from_bot = map(
        hmap, (left_to_top, left_to_bot, right_from_top, right_from_bot))
    mid_even_tb = [mid_even_tb[2*j+1] + mid_even_tb[2*j] * PMAP[1]
                   for j in range(n)]
    mid_odd_tb = [mid_odd_tb[2*j+1] + mid_odd_tb[2*j] * PMAP[1]
                  for j in range(n)]
...
def gridlandProvinces(s1: str, s2: str) -> int:
